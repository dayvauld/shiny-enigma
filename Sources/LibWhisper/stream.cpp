...
stream_context *stream_init(stream_params params) {
    ...
    std::string filename = std::string(params.audio_filename);// + ".wav";
    
    fprintf(stderr, "%s: attempt to open file %s :\n", __func__, params.audio_filename);

    ctx->wav_writer.open(filename, WHISPER_SAMPLE_RATE, 16, 1);
...
int stream_run(stream_context *ctx, void *callback_ctx, stream_callback_t callback) {
    ...
    ctx->wav_writer.write(ctx->pcmf32_new.data(), ctx->pcmf32_new.size());
