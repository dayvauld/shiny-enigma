...
var cacheDirectory: URL {
    ...
    return parent.appendingPathComponent(Bundle.main.bundleIdentifier!)
}
...
public class WhisperStream: Thread {
    ...
    public init(model: URL, device: CaptureDevice? = nil, window: TimeInterval = 300) {
        ...
        self.audioFilename = cacheDirectory.appendingPathComponent("\(dateInStringFormat).wav")
    ...
    func task() {
      audioFilename.path.withCString{ filenameCStr in
        print("c string \(String(cString: filenameCStr))")
      
        model.path.withCString { modelCStr in
            var params = stream_default_params()
            params.model = modelCStr
            params.audio_filename = filenameCStr
